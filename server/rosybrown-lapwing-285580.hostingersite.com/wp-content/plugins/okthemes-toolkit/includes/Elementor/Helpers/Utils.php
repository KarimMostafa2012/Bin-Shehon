...
class Utils {
	...
	public static function get_page_title( $include_context = true ) {
		...
		if ( is_singular() ) {
			...
			/* translators: %s: Search term. */
			if ( $include_context ) {
				$title = sprintf( __( 'Search Results for: %s', 'okthemes-toolkit' ), get_search_query() );
			} else {
			...
			if ( get_query_var( 'paged' ) ) {
				...
				$title .= sprintf( __( '&nbsp;&ndash; Page %s', 'okthemes-toolkit' ), get_query_var( 'paged' ) );
			}
		} elseif ( is_category() ) {
			...
			if ( $include_context ) {
				...
				$title = sprintf( __( 'Category: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_tag() ) {
			$title = single_tag_title( '', false );
			if ( $include_context ) {
				/* translators: Tag archive title. 1: Tag name */
				$title = sprintf( __( 'Tag: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_author() ) {
			...
			if ( $include_context ) {
				...
				$title = sprintf( __( 'Author: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_year() ) {
			$title = get_the_date( _x( 'Y', 'yearly archives date format', 'okthemes-toolkit' ) );

			if ( $include_context ) {
				/* translators: Yearly archive title. 1: Year */
				$title = sprintf( __( 'Year: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_month() ) {
			$title = get_the_date( _x( 'F Y', 'monthly archives date format', 'okthemes-toolkit' ) );

			if ( $include_context ) {
				/* translators: Monthly archive title. 1: Month name and year */
				$title = sprintf( __( 'Month: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_day() ) {
			$title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'okthemes-toolkit' ) );

			if ( $include_context ) {
				/* translators: Daily archive title. 1: Date */
				$title = sprintf( __( 'Day: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_tax( 'post_format' ) ) {
			if ( is_tax( 'post_format', 'post-format-aside' ) ) {
				$title = _x( 'Asides', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
				$title = _x( 'Galleries', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
				$title = _x( 'Images', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
				$title = _x( 'Videos', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
				$title = _x( 'Quotes', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
				$title = _x( 'Links', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
				$title = _x( 'Statuses', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
				$title = _x( 'Audio', 'post format archive title', 'okthemes-toolkit' );
			} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
				$title = _x( 'Chats', 'post format archive title', 'okthemes-toolkit' );
			}
			...
			if ( $include_context ) {
				...
				$title = sprintf( __( 'Archives: %s', 'okthemes-toolkit' ), $title );
			}
		} elseif ( is_tax() ) {
			...
			if ( $include_context ) {
				...
				$title = sprintf( __( '%1$s: %2$s', 'okthemes-toolkit' ), $tax->labels->singular_name, $title );
			}
		} elseif ( is_archive() ) {
			$title = __( 'Archives', 'okthemes-toolkit' );
		} elseif ( is_404() ) {
			$title = __( 'Page Not Found', 'okthemes-toolkit' );
		} 
	...
	public static function get_the_posts( $post = [] ) {
		...
		if ( ! empty ( get_posts( [ 'post_type' => $post ] ) ) ) {
            foreach( get_posts( [ 'post_type' => $post, 'posts_per_page' => -1 ] ) as $value ) {
                $output[$value->post_type . '-' . $value->ID] = __( ucfirst( $value->post_type ) .  ': ', 'okthemes-toolkit' ) . $value->post_title;
            }
		}
	 ...
	 * @since 1.0.7.7
	 */
	public static function get_image_hover_animations() {
		$output = [
            '' => __( 'None', 'okthemes-toolkit' ),
            'zoom-in' => __( 'Zoom In', 'okthemes-toolkit' ),
            'zoom-out' => __( 'Zoom Out', 'okthemes-toolkit' ),
			'tilt-right' => __( 'Tilt Right', 'okthemes-toolkit' ),

        ];

		return apply_filters( 'neuron/utils/image_hover_animations', $output );
	}
