<?php
/**
 * orologio WooCommerce Shop filters Class
 *
 * @package  orologio
 * @since    1.0.0
 */
...
class orologio_ShopFilter {
    ...
    public function filter_scripts() {
        ...
        wp_enqueue_script('orologio-ajax-filter', get_template_directory_uri() . '/lib/woocommerce/js/custom.js', array('jquery'), $orologio_version, true);

        wp_localize_script('orologio-ajax-filter', 'ajax_filter_params', array(
    ...
    public function ajax_filter_products() {
        ...
        if ( ! isset( $_POST['ajax_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['ajax_nonce'] ) ), 'ajax_filter_nonce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid nonce verification', 'orologio' ) ) );
            return;
        }
    ...
    protected function render_product_loop( $products ) {
        if ( ! empty( $products ) ) {
            ...
            echo '<li>'._e( 'No products found.', 'orologio' ).'</li>';
        }
    }
    ...
    protected function generate_filters_html($product_ids, $selected_categories, $selected_tags, $selected_attributes, $current_category_slug, $current_tag_slug) {
        ...
        $product_count_enabled = Orologio_Helper::get_option('store_filter_product_count', 'enabled');
    ...
    public function render_filters() {
        // Theme options
        $filter_label = Orologio_Helper::get_option('store_filter_label', 'Filter the wines');
        $clear_label = Orologio_Helper::get_option('store_filter_clear_label', 'Clear filters');
        $active_filters = Orologio_Helper::get_option('store_filter_active_filters', 'enabled');
        $product_count = Orologio_Helper::get_option('store_filter_product_count', 'enabled'); // Check product count option
        ...
        <div class="filters-container">
            ...
            <div class="filter-section">
                <h6><?php _e( 'Categories', 'orologio' ); ?></h6>
