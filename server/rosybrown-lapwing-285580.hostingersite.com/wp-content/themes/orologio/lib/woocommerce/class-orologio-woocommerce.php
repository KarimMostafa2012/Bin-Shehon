<?php
/**
 * orologio WooCommerce Class
 *
 * @package  orologio
 * @since    2.0.0
 */
namespace OrologioTheme\Classes;
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

if ( ! class_exists( 'orologio_WooCommerce' ) ) :

	/**
	 * The orologio WooCommerce Integration class
	 */
	class orologio_WooCommerce {
		...
		public function setup() {
			add_theme_support(
				'woocommerce', apply_filters(
					'orologio_woocommerce_args', array(
		...
		public function woocommerce_scripts() {
			global $orologio_version;
			wp_enqueue_style( 'orologio-fontawesome', get_template_directory_uri() . '/assets/fontawesome/css/fontawesome.min.css', '', $orologio_version );
            wp_enqueue_style( 'orologio-fontawesome-regular', get_template_directory_uri() . '/assets/fontawesome/css/solid.min.css', '', $orologio_version );
    		wp_enqueue_style( 'orologio-woocommerce-style', get_template_directory_uri() . '/assets/css/woocommerce.css', array(), $orologio_version );
		}
		...
		public function related_products_args( $args ) {
			$args = apply_filters(
				'orologio_related_products_args', array(
					'posts_per_page' => 3,
					'columns'        => 3,
				)
			);
        ...
        public function upsell_products_args( $args ) {
            $args = apply_filters(
                'orologio_upsell_products_args', array(
                    'posts_per_page' => 3,
                    'columns'        => 3,
                )
            );
		...
		public function thumbnail_columns() {
			...
			return intval( apply_filters( 'orologio_product_thumbnail_columns', $columns ) );
		}
		...
		public function change_breadcrumb_delimiter( $defaults ) {
			...
			$defaults['wrap_before'] = '<div class="orologio-breadcrumb"><div class="col-full"><nav class="woocommerce-breadcrumb">';
		...
		public static function render_menu_cart_toggle_button($customIcon) { ?>
			<div class="cart-drawer-wrapper">
				<a id="cart-drawer-trigger" href="<?php echo esc_url(wc_get_cart_url()); ?>" title="<?php esc_attr_e('View your shopping cart', 'orologio'); ?>">
				<?php
					if (!empty($customIcon)) {
						echo wp_kses($customIcon, orologio_kses_allowed_html(array(), 'icon'));
					} else {
						echo orologio_get_icons('header-minicart-v2');
					}
		...
		public static function render_menu_cart($customIcon = '') {
			...
			if (class_exists('\OKThemes\Toolkit\WooCommerce\CartDrawer')) {
				return; // Let plugin handle it
			}
				...
				<div id="cartDrawer">
					<div class="cart-drawer-container">
						<div class="cart-drawer-header">
							<h4><?php esc_html_e( 'Shopping cart', 'orologio' ); ?></h4>
							<a href="#" id="closeDrawerbtn" title="<?php esc_attr_e( 'Close', 'orologio' ); ?>">
								<?php echo orologio_get_icons('shopping-cart-close'); ?>
							</a>
						</div>
...
return new orologio_WooCommerce();
